...
- **地形系统**：平原、林地（+10防御）、营地（+5防御）、河流（半阻断，可达但不可穿过）、城池（+15防御）、山地（全阻断，不可进入，弓兵站上去攻击范围+1）
...
| 城池 | `w` | +15 | 占领后额外+8防御 |
| 山地 | `mt` | +15 | **全阻断**：不可进入、不可穿过、完全阻挡视线；弓兵站上去攻击范围+1 |
...
| 对象 | 默认阻断模式 | 说明 |
|------|:---:|------|
| 山地 `mt` | `full` 全阻断 | 不可进入、不可穿过、完全阻挡视线 |
...
const cells = Range.cellsInRangeWithBlock('+', 4, actor.x, actor.y, {
  ...
  blockFilter: (x, y, piece, terrain) => {
    ...
    // 返回 undefined → 回退默认（山地 full / 敌方 full / 其他 none）
  }
});
```
